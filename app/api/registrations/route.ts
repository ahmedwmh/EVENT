...
export async function GET(request: NextRequest) {
  try {
    ...
    const formattedRegistrations = registrations.map((reg) => ({
      ...
      city: reg.city,
      message: reg.message || "",
      firstPersonName: reg.firstPersonName || "",
      secondPersonName: reg.secondPersonName || "",
      invitationSent: reg.invitationSent || false,
      familyAccepted: reg.familyAccepted || false,
      notes: reg.notes || "",
      attended: reg.attended || false,
      otpCode: reg.otpCode || "",
      invitationCode: reg.invitationCode || "",
      qrCodeScanned: reg.qrCodeScanned || false,
      qrCodeScannedAt: reg.qrCodeScannedAt?.toISOString() || "",
